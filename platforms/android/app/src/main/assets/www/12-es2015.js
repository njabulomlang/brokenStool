...
let BasketPage = class BasketPage {
    constructor(NavCtrl, alertCtrl, router, toastCtrl) {
    ...
    placeOrder(info) {
        ...
        if (this.prodCart.length === 0) {
            ...
            this.dbOrder.doc(docname).set({ product: myArr, timestamp: new Date().getTime(), status: 'received', userID: firebase__WEBPACK_IMPORTED_MODULE_2__["auth"]().currentUser.uid, totalPrice: this.getTotal() }).then(() => {
                doc.forEach((id) => {
